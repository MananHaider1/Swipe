...
class _EditProfilePageState extends ConsumerState<EditProfilePage> {
  ...
  Uint8List? _profilePictureFile;
  final List<String> _interests = [];
  final List<dynamic> _medias = [
  ...
  void initState() {
    ...
    if (kDebugMode) {
      print("Images: ${widget.userProfileModel.mediaFiles.length}");
    }
  ...
  //     if (widget.userProfileModel.mediaFiles.length > i) {
  //       _medias[i] =
  //           await getUint8ListFromUrl(widget.userProfileModel.mediaFiles[i]);
  //       if (kDebugMode && _medias[i] != null) {
  //         print("Image Added: ${widget.userProfileModel.mediaFiles[i]}");
  //       }
  //     }
  //   }
  // }
  ...
  void _onSave() async {
    ...
    for (var media in _medias) {
      if (media is String && Uri.parse(media).hasScheme) {
        mediaURLs.add(media);
      } else if (media == null) {
    ...
    Future<String?> uploadProfilePictureWeb(String phoneNumber) async {
      final storageRef = FirebaseStorage.instance.ref();

      final imageRef = storageRef.child(
          "user_profile_pictures/$phoneNumber${DateTime.now().millisecondsSinceEpoch}.jpeg");
      // .child(
      ...
      final uploadTask = imageRef.putData(
          _profilePictureFile!, SettableMetadata(contentType: 'image/jpeg'));
      await uploadTask.whenComplete(() async {
        imageUrl = await imageRef.getDownloadURL();
        // });
      });
    ...
    if (_profilePictureFile != null) {
      profilePicUrl = await uploadProfilePictureWeb(
    ...
    if (_formKey.currentState!.validate()) {
      final newUserProfileModel = widget.userProfileModel.copyWith(
        ...
        profilePicture: profilePicUrl ?? _profilePicture,
                      ...
                      child: GestureDetector(
                        onTap: () async {
                          void setProfilePicture() async {
                            final imagePath = await pickMediaAsBytes();
                            if (imagePath != null) {
                              setState(() {
                                _profilePictureFile = imagePath;
                              });
                            }
                          }
                              ...
                              : _profilePicture == null ||
                                  ...
                                  ? CircleAvatar(
                                      ...
                                      child: const Icon(
                                        ...
                                        size: AppConstants.defaultNumericValue *
                                            7,
                                      ),
                                    )
